...
// Remembers the currently selected color while the create-label form is open
let selectedSwatchColor = SWATCH_COLORS[0];


// Finds a card object anywhere on the board by id
...
// Finds which list currently contains a given card id
...
// Render the available labels in the modal
function renderLabelPicker(card) {
  labelPicker.innerHTML = "";

  // --- Existing label chips (togglable) ---
...
function renderCreateLabelForm(card) {
  ...
  formEl
    ...
    .addEventListener("click", () => {
      ...
      // Add the new label to the palette
      LABEL_PALETTE.push({
        name: name,
        color: selectedSwatchColor
      });

      // Automatically apply the new label to this card
      card.labels.push(name);

      // Rebuild the label picker
      renderLabelPicker(card);
    });


  // --- Cancel creating label ---
  formEl
    .querySelector(".cancel-btn")
    .addEventListener("click", () => {

      // Discard the form without changing the data
      renderLabelPicker(card);
    });


  // Replace "+ Create label" with the form
  const createBtn = labelPicker.querySelector(".create-label-btn");

  createBtn.replaceWith(formEl);

  // Focus the label name input
